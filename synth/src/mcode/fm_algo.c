                              ...
                              void *p_code_rw, const void **code_x)
{
    ...
    fail |= x64_movsd(&code_rw, X64_REG(XMM0), X64_REG(XMM1));
    fail |= x64_movsd(&code_rw, X64_REG(XMM0), X64_INDIRECT_ISC(RAX, RDX, 0, 64));
...
#else
#error FMGen not supported on this architecture
#endif
